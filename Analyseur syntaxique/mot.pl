mot(tenir,verbe,venir, "tien", 1).
mot(tenir,verbe,venir, "ten", 2).
mot(tenir,verbe,venir, "tienn", 3).
mot(tenir,verbe,venir, "tin", 4).
mot(tenir,verbe,venir, "tîn", 5).

mot(le, article, le, "l", 1).

mot(balle, nom, balle, "balle", 1).

mot(bleu, adjectif, bleu, "bleu", 1).
mot(vert, adjectif, bleu, "vert", 1).

mot(mais, coordination, conjnul, "mais", 1).
mot(et, coordination, conjnul, "et", 1).

mot(qui, relative, conjnul, "qui", 1).